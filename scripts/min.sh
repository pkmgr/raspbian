...
# @@File             :  min.sh
# @@Description      :  Script to setup min for Raspbian
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set functions
PKMGR_SCRIPTSFUNCTURL="${PKMGR_SCRIPTSFUNCTURL:-https://github.com/casjay-dotfiles/scripts/raw/main/functions}"
PKMGR_SCRIPTSFUNCTDIR="${PKMGR_SCRIPTSFUNCTDIR:-/usr/local/share/CasjaysDev/scripts}"
PKMGR_SCRIPTSFUNCTFILE="${PKMGR_SCRIPTSFUNCTFILE:-system-installer.bash}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if [ -f "../functions/$PKMGR_SCRIPTSFUNCTFILE" ]; then
	. "../functions/$PKMGR_SCRIPTSFUNCTFILE"
elif [ -f "$PKMGR_SCRIPTSFUNCTDIR/functions/$PKMGR_SCRIPTSFUNCTFILE" ]; then
	. "$PKMGR_SCRIPTSFUNCTDIR/functions/$PKMGR_SCRIPTSFUNCTFILE"
else
	curl -LSs "$PKMGR_SCRIPTSFUNCTURL/$PKMGR_SCRIPTSFUNCTFILE" -o "/tmp/$PKMGR_SCRIPTSFUNCTFILE" || exit 1
	. "/tmp/$PKMGR_SCRIPTSFUNCTFILE"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
SCRIPT_OS="Raspbian"
...
RELEASE_TYPE="$(. /etc/os-release 2>/dev/null; { [[ " $ID_LIKE " == *debian* ]] || [ "$ID" = "debian" ]; } && echo "raspbian")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if ! grep --no-filename -sE -- '^ID=|^ID_LIKE=|^NAME=' /etc/*-release | grep -qiwE -- "raspbian"; then
...
__retrieve_version_file() { __grab_remote_file "https://github.com/casjay-base/raspbian/raw/main/version.txt" | head -n1 || echo "Unknown version"; }
__domain_name() {
	local d="" f=""
...
# __install_pkg cronie-noanacron  # skipped on raspbian
__install_pkg postfix
__install_pkg net-tools
__install_pkg wget
__install_pkg curl
__install_pkg git
__install_pkg mailutils
...
# __install_pkg biosdevname  # skipped on raspbian
...
# __install_pkg cockpit-bridge  # skipped on raspbian
# __install_pkg cockpit-system  # skipped on raspbian
# __install_pkg cockpit-ws  # skipped on raspbian
...
# __install_pkg grub2-tools-extra  # skipped on raspbian
# __install_pkg grubby  # skipped on raspbian
__install_pkg gzip
...
# __install_pkg mod_geoip  # skipped on raspbian
# __install_pkg mod_http2  # skipped on raspbian
# __install_pkg mod_maxminddb  # skipped on raspbian
__install_pkg libapache2-mod-perl2
# __install_pkg mod_ssl  # skipped on raspbian
__install_pkg libapache2-mod-wsgi-py3
# __install_pkg mod_proxy_html  # skipped on raspbian
...
# __install_pkg perl-DBD-Firebird  # skipped on raspbian
...
__install_pkg postfix
# __install_pkg postfix-pcre  # skipped on raspbian
__install_pkg python3-certbot-dns-rfc2136
__install_pkg python3-configargparse
__install_pkg python3-cryptography
# __install_pkg python3-enum34  # skipped on raspbian
# __install_pkg python3-funcsigs  # skipped on raspbian
__install_pkg python3-future
__install_pkg python3-idna
# __install_pkg python3-josepy  # skipped on raspbian
# __install_pkg python3-mock  # skipped on raspbian
__install_pkg python3-pynvim
# __install_pkg python3-parsedatetime  # skipped on raspbian
# __install_pkg python3-pbr  # skipped on raspbian
__install_pkg python3-pip
__install_pkg python3-psutil
# __install_pkg python3-pyasn1  # skipped on raspbian
# __install_pkg python3-pyrfc3339  # skipped on raspbian
# __install_pkg python3-pysocks  # skipped on raspbian
__install_pkg python3-requests
# __install_pkg python3-six  # skipped on raspbian
__install_pkg python3-virtualenv
__install_pkg libreadline-dev
# __install_pkg rootfiles  # skipped on raspbian
__install_pkg rsync
...
devnull git clone -q "https://github.com/casjay-base/raspbian" "$CONFIG_TEMP_DIR"
...
run_post_message="Installing default server files" \
	__run_post sudo -HE STATICSITE="$(hostname -f)" bash -c "$(curl -LSs "https://github.com/casjay-templates/default-web-assets/raw/main/setup.sh")"
...
if [ -n "$CLOUDFLARE_EMAIL" ] && [ -n "$CLOUDFLARE_API_KEY" ] && [ -n "$CLOUDFLARE_ZONE_NAME" ] && type -P cloudflare >/dev/null 2>&1; then
	...
	if devnull cloudflare update "$PKMGR_SET_HOSTNAME" "${cf_args[@]}"; then
		CLOUDFLARE_DOMAIN="yes"
		devnull cloudflare update "*.$PKMGR_SET_HOSTNAME" "${cf_args[@]}"
		printf_blue "Successfully updated $PKMGR_SET_HOSTNAME in $CLOUDFLARE_ZONE_NAME"
	elif devnull cloudflare create "$PKMGR_SET_HOSTNAME" "${cf_args[@]}"; then
		CLOUDFLARE_DOMAIN="yes"
		devnull cloudflare create "*.$PKMGR_SET_HOSTNAME" "${cf_args[@]}"
		printf_blue "Created $PKMGR_SET_HOSTNAME for $CLOUDFLARE_ZONE_NAME"
	else
		printf_red "Failed to create record $PKMGR_SET_HOSTNAME for zone $CLOUDFLARE_ZONE_NAME"
	fi
	unset cf_args
fi
